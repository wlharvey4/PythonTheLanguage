...
*** NotImplemented
    ...
    #+cindex:@{true}, @code{NotImplemented}
...
#+cindex:collections.queue
*** Lists as Queues

- Queue :: the first element added is the first element retrieved (“first-in,
           first-out”).  Lists are not efficient for this purpose.  While
           appends and pops from the end of list are fast, doing inserts or
...
- ~collections.queue~ :: To implement a queue, use [[https://docs.python.org/3.7/library/collections.html#collections.deque][collections.deque]] which was
...
** Lambda Expressions (lambda form)
   ...
   Lambda expressions[fn:3] (sometimes called lambda forms) are used to create
...
# Local Variables:
# time-stamp-pattern:"8/^\\#\\+\\(DATE\\|date\\):\s?%:y-%02m-%02d %02H:%02M$"
